...
const ResultsPanel = ({ result, isLoading, onRunQuery, queryConfig, onConfigChange, chartConfig, onChartConfigChange }) => {
    ...
    return (
        <div className="vsqb-results-shell">
            <div className="vsqb-results-header">
                <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-sm">Actions & Results</h3>
                    {canShowChart && (
                ...
                <div className="flex items-center gap-2">
                    <button id="run-query-btn" onClick={onRunQuery} disabled={isLoading} className="btn-primary">
                        <Play size={16} /> <span>{isLoading ? 'Running...' : 'Run Query'}</span>
                    </button>
                     <button onClick={saveQuery} className="vsqb-navy-button" title="Save Query"><Save size={16} /></button>
                     <button onClick={() => setShowSaved(true)} className="vsqb-navy-button">Load Saved</button>
